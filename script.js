...
let currentPage = 1;
const itemsPerPage = window.innerWidth <= 768 ? 24 : 24;
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    // Load posts from window.POSTS_DATA
    if (window.POSTS_DATA && window.POSTS_DATA.length > 0) {
        movieData = [...window.POSTS_DATA].reverse(); // Reverse to show newest posts first
        console.log(`📊 Loaded ${movieData.length} posts from posts-data.js`);
    } else {
        console.warn('⚠️ No posts found in posts-data.js');
    }
...
function createMovieCard(movie) {
    ...
    // Check if format contains "Movie" or "Film" to add movie-badge class
    const formatText = movie.format || movie.season || '';
    const formatClass = formatText.toLowerCase().includes('movie') || formatText.toLowerCase().includes('film') ? 'season-badge movie-badge' : 'season-badge';
    ...
    card.innerHTML = `
        ...
        <div class="movie-info">
            ...
            <div class="movie-meta">
                ...
                <span class="${formatClass}">${formatText}</span>
            </div>
        </div>
    `;
...
function updateHeroStats() {
    // Count posts from featured movieData array
    const postsCount = movieData.length; // Dynamically counts all featured posts
    
    // Count unique voice artists from POSTS_DATA
    let artistsCount = 0;
    if (window.POSTS_DATA && Array.isArray(window.POSTS_DATA)) {
        const uniqueArtists = new Set();
        
        window.POSTS_DATA.forEach(post => {
            // Count cast members (voice actors)
            if (post.cast && Array.isArray(post.cast)) {
                post.cast.forEach(castMember => {
                    if (castMember.actorName) {
                        uniqueArtists.add(castMember.actorName);
                    }
                });
            }
        });
        ...
        // Fallback count if data not loaded
        artistsCount = 5;
    }
    ...
    // Animate counting for voice artists
...
let currentSlide = 0;
let slideInterval;
let slides = []; // Will be populated from HTML
...
function initPosterSlider() {
    ...
    // Read image URLs from HTML
    slides = Array.from(carouselSlides).map(slide => {
...
// Function to protect all images
...
// Re-protect images when new content is loaded
