...
function loadRatings() {
    ...
    // Try to get posts from POSTS_DATA first, then postsManager
    let posts = [];
    
    if (window.POSTS_DATA && window.POSTS_DATA.length > 0) {
        posts = window.POSTS_DATA;
        console.log(`📊 Loaded ${posts.length} posts from posts-data.js for ratings page`);
    } else if (window.postsManager) {
        posts = window.postsManager.getAllPosts();
        console.log(`📊 Loaded ${posts.length} posts from postsManager for ratings page`);
    } else {
        container.innerHTML = '<div class="empty-state"><i class="fas fa-exclamation-circle"></i><p>No posts data available</p></div>';
        return;
    }
...
function createRatingCard(post, rank) {
    ...
    // Get format info (Season 1, Season 01, etc.)
    const formatInfo = post.format || '';
    ...
    card.innerHTML = `
        ...
        <div class="anime-title-section">
            ...
            ${formatInfo ? `<div class="anime-season">${formatInfo}</div>` : ''}
        </div>
