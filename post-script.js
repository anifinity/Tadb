// Load detailed movie data from posts-data.js (shared with index.html)
let detailedMovieData = {};

// Check if POSTS_DATA is available from posts-data.js
if (window.POSTS_DATA && window.POSTS_DATA.length > 0) {
    // Convert array to object with ID as key for easy lookup
    window.POSTS_DATA.forEach(post => {
        detailedMovieData[post.id] = post;
    });
    console.log(`📊 Loaded ${window.POSTS_DATA.length} posts from posts-data.js for post page`);
}

// Legacy data structure (fallback if posts-data.js not loaded)
const legacyDetailedMovieData = {
    1: {
        id: 1,
        title: "Zenshu",
        year: "2025",
        ...
        statistics: {
            rating: "6.9",
            likedPercentage: "94%",  // This is Crunchyroll percentage
            totalViews: "4.2M"
        },
    ...
    2: {
        ...
        statistics: {
            rating: "8.3",
            likedPercentage: "92%",  // This is Crunchyroll percentage
            totalViews: "2.1M"
        },
...
// Theme elements - Now handled by theme-manager.js
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    setupEventListeners();
    // initializeTheme(); - Now handled by theme-manager.js
});
...
function loadMovieData(movieId) {
    // First try to load from posts-data.js (shared with index.html)
    currentMovie = detailedMovieData[movieId];
    
    // If not found, try legacy data
    if (!currentMovie) {
        currentMovie = legacyDetailedMovieData[movieId];
    }
    
    // If still not found, use default
    if (!currentMovie) {
        currentMovie = { ...defaultMovieData, id: movieId };
        console.warn(`⚠️ Post ID ${movieId} not found in posts-data.js or legacy data`);
    } else {
        console.log(`✅ Loaded post ID ${movieId} from ${detailedMovieData[movieId] ? 'posts-data.js' : 'legacy data'}`);
    }
    ...
    // Update Overview tab synopsis with spoiler alert
    const overviewSynopsis = document.getElementById('overviewSynopsis');
    if (overviewSynopsis) {
        // Apply spoiler blur if enabled
        if (currentMovie.spoilerAlert === "visible") {
            // Wrap synopsis in a blurred container
            overviewSynopsis.innerHTML = `
                <div class="spoiler-container">
                    <div class="spoiler-blur-text">${currentMovie.synopsis}</div>
                    <div class="spoiler-badge">
                        <div class="spoiler-badge-main">⚠️ SPOILERS ALERT</div>
                        <div class="spoiler-badge-sub">Click to reveal</div>
                    </div>
                </div>
            `;
            
            overviewSynopsis.style.cursor = 'pointer';
            overviewSynopsis.title = 'Click to reveal spoilers';
            
            // Add click to reveal
            overviewSynopsis.onclick = function() {
                const container = this.querySelector('.spoiler-container');
                if (container) {
                    container.innerHTML = currentMovie.synopsis;
            ...
            overviewSynopsis.style.cursor = 'default';
            overviewSynopsis.onclick = null;
        }
    }
    ...
    if (currentMovie.statistics) {
        ...
        if (statLiked) {
            // Use likedPercentage (which is Crunchyroll percentage)
    ...
    // Update runtime in Overview tab
    const episodeCount = document.getElementById('episodeCount');
    if (episodeCount) {
        episodeCount.textContent = currentMovie.duration;
    }
...
function loadDetails() {
    ...
    // Handle Total Episodes box
    const totalEpisodesBox = document.getElementById('totalEpisodesBox');
    const totalEpisodesValue = document.getElementById('totalEpisodesValue');
    
    if (totalEpisodesBox && totalEpisodesValue) {
        if (currentMovie.totalEpisodes && currentMovie.totalEpisodes.toLowerCase() !== 'none') {
            totalEpisodesValue.textContent = currentMovie.totalEpisodes;
            totalEpisodesBox.style.display = 'flex';
        } else {
            totalEpisodesBox.style.display = 'none';
        }
    }
}
...
function createCastElement(member) {
    ...
    castDiv.innerHTML = `
        ...
        <div class="cast-right">
            <div class="character-info">
                ...
                <div class="character-dub">${member.platforms && member.platforms.length > 0 ? member.platforms.join(', ') : (member.platform || currentMovie.title)}</div>
            </div>
    ...
    castDiv.addEventListener('click', () => {
        showCastCrewPopup({
            ...
            role: 'Voice Artist',
            avatar: member.actorImage,
            character: member.characterName,
            platforms: member.platforms && member.platforms.length > 0 ? member.platforms.join(', ') : (member.platform || currentMovie.title),
...
function createCrewElement(member) {
    ...
    crewDiv.innerHTML = `
        ...
        <div class="cast-right">
            <div class="character-info">
                ...
                ${member.platforms && member.platforms.length > 0 ? `<div class="character-dub">${member.platforms.join(', ')}</div>` : (member.platform ? `<div class="character-dub">${member.platform}</div>` : '')}
            </div>
    ...
    // Add click handler to show popup
    crewDiv.addEventListener('click', () => {
        showCastCrewPopup({
            name: member.name,
            role: member.role,
            avatar: member.avatar,
            project: animeTitle,
            platforms: member.platforms && member.platforms.length > 0 ? member.platforms.join(', ') : (member.platform || ''),
            description: member.description || '',
            type: 'crew'
        });
    });
    ...
    platformLink.innerHTML = `
        <div class="platform-card">
            <div class="platform-logo">
                <img src="${platform.logo || platform.icon || ''}" alt="${platform.name}" onerror="this.style.display='none'">
            </div>
            <div class="platform-info">
                <h4 class="platform-name">${platform.name}</h4>
                <p class="platform-description">${platform.status || 'Available'}</p>
            </div>
...
function setupEventListeners() {
    ...
    // Theme toggle - Now handled by theme-manager.js
    // if (themeToggle) {
...
// Theme System Functions - Now handled by theme-manager.js
...
function updateThemeIcon(theme) {
    // Theme icon update is now handled by theme-manager.js
    // This function is kept for compatibility but does nothing
}
...
function updateMobilePopupContent() {
    ...
    // Update IMDb rating from statistics
    if (popupRating && currentMovie.statistics) {
        popupRating.textContent = currentMovie.statistics.rating;
    }
    
    // Update TMDB rating from statistics (use likedPercentage)
    if (popupTmdbRating && currentMovie.statistics) {
        popupTmdbRating.textContent = currentMovie.statistics.likedPercentage;
    }
    
    // Update view count
    if (popupViews) {
        popupViews.textContent = 'Total Ratings';
    }
    ...
    // Add trailer button functionality (new YouTube trailer button)
    if (trailerBtn) {
...
// Cast/Crew Popup Functions
function showCastCrewPopup(data) {
    const popup = document.getElementById('castCrewPopup');
    ...
    if (data.type === 'cast') {
        detailsHTML = `
            ...
            <p><strong>Platform:</strong> ${data.platforms}</p>
            <p><strong>Anime:</strong> ${currentMovie.title}</p>
            ${data.description ? `
        ...
        detailsHTML = `
            ...
            ${data.platforms ? `<p><strong>Platform:</strong> ${data.platforms}</p>` : ''}
...
// Prevent right-click on images (but allow screenshots)
...
// Prevent mousedown right-click
document.addEventListener('mousedown', function(e) {
    if ((e.target.tagName === 'IMG' || e.target.closest('img')) && e.button === 2) {
        e.preventDefault();
        e.stopPropagation();
        return false;
    }
